...
const TABLE = { "tf-clients": "clients", "tf-tasks": "tasks", "tf-pages": "pages", "tf-tags": "tags", "tf-meetings": "meetings", "tf-activity": "activity", "tf-users": "tf_users" };
...
const SEED = {
  users: [
    { id: "u1", name: "Franco", email: "franco@nistaestudio.com", password: "franco123", avatar: "FR", color: "#5b6af0", role: "admin",  allowedClients: [] },
    { id: "u2", name: "Pablo",  email: "pablo@nistaestudio.com",  password: "pablo123",  avatar: "PA", color: "#e86c4a", role: "member", allowedClients: [] },
    { id: "u3", name: "Nico",   email: "nico@nistaestudio.com",   password: "nico123",   avatar: "NI", color: "#3db88a", role: "member", allowedClients: [] },
    { id: "u4", name: "Lucho",  email: "lucho@nistaestudio.com",  password: "lucho123",  avatar: "LU", color: "#a855f7", role: "member", allowedClients: [] },
  ],
  clients: [
...
const statusMap    = { todo: { label: "Por hacer", color: "#707070" }, doing: { label: "En progreso", color: "#5b6af0" }, blocked: { label: "Bloqueada", color: "#e86c4a" }, done: { label: "Completado", color: "#3db88a" } };
const clientStatusMap = { active: { label: "Activo", color: "#3db88a" }, paused: { label: "En pausa", color: "#f0a030" }, closed: { label: "Cerrado", color: "#555" } };
...
const Modal = ({ title, onClose, children, wide }) => (
  <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(6px)", zIndex: 1000, display: "flex", alignItems: "flex-end", justifyContent: "center", padding: 0 }}
    onClick={e => e.target === e.currentTarget && onClose()}>
    <div className={`tf-modal-inner${wide ? " tf-modal-wide" : ""}`} style={{ background: "#161616", border: "1.5px solid #222", width: "100%", maxHeight: "92vh", overflow: "auto", boxShadow: "0 -8px 60px rgba(0,0,0,0.6)" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "22px 24px 0" }}>
        <h3 style={{ margin: 0, fontSize: 17, fontWeight: 700, fontFamily: "inherit", color: "#f0f0f0" }}>{title}</h3>
        <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: "#555", padding: 4 }}><Icon name="x" /></button>
      </div>
...
const Input = ({ label, ...props }) => (
  <div style={{ marginBottom: 16 }}>
    {label && <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#b0b0b0", marginBottom: 6, letterSpacing: "0.04em", textTransform: "uppercase" }}>{label}</label>}
    <input {...props} style={{ width: "100%", padding: "10px 12px", border: "1.5px solid #2a2a2a", borderRadius: 8, fontSize: 14, fontFamily: "inherit", outline: "none", boxSizing: "border-box", background: "#1e1e1e", ...props.style }}
      onFocus={e => e.target.style.borderColor = "#5b6af0"} onBlur={e => e.target.style.borderColor = "#2a2a2a"} />
  </div>
);
const Textarea = ({ label, rows = 4, ...props }) => (
  <div style={{ marginBottom: 16 }}>
    {label && <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#b0b0b0", marginBottom: 6, letterSpacing: "0.04em", textTransform: "uppercase" }}>{label}</label>}
    <textarea rows={rows} {...props} style={{ width: "100%", padding: "10px 12px", border: "1.5px solid #2a2a2a", borderRadius: 8, fontSize: 14, fontFamily: "inherit", outline: "none", resize: "vertical", boxSizing: "border-box", background: "#1e1e1e", ...props.style }}
      onFocus={e => e.target.style.borderColor = "#5b6af0"} onBlur={e => e.target.style.borderColor = "#2a2a2a"} />
  </div>
);
const Select = ({ label, children, ...props }) => (
  <div style={{ marginBottom: 16 }}>
    {label && <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#b0b0b0", marginBottom: 6, letterSpacing: "0.04em", textTransform: "uppercase" }}>{label}</label>}
    <select {...props} style={{ width: "100%", padding: "10px 12px", border: "1.5px solid #2a2a2a", borderRadius: 8, fontSize: 14, fontFamily: "inherit", outline: "none", boxSizing: "border-box", background: "#1e1e1e", cursor: "pointer" }}>{children}</select>
  </div>
);
const Btn = ({ children, variant = "primary", onClick, style = {}, disabled }) => {
  const styles = { primary: { background: "#5b6af0", color: "#fff", border: "none" }, secondary: { background: "#1e1e1e", color: "#b0b0b0", border: "1.5px solid #2a2a2a" }, danger: { background: "#2a1008", color: "#e86c4a", border: "1.5px solid #3a1a0a" }, accent: { background: "#5b6af015", color: "#5b6af0", border: "1.5px solid #5b6af030" } };
...
const LogoUploader = ({ client, onUpload }) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
      ...
      <button onClick={() => ref.current.click()} style={{ background: "none", border: "1.5px dashed #ddd", borderRadius: 8, padding: "6px 14px", fontSize: 12, color: "#707070", cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 6 }}>
...
const TagSelector = ({ clientId, tags, selected, onChange, onDeleteTag }) => {
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#b0b0b0", marginBottom: 8, letterSpacing: "0.04em", textTransform: "uppercase" }}>Etiquetas</label>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 8 }}>
        {clientTags.length === 0 && !adding && <span style={{ fontSize: 12, color: "#444" }}>Sin etiquetas para este cliente.</span>}
        {clientTags.map(tag => {
      ...
      {adding ? (
        <div style={{ display: "flex", gap: 8, alignItems: "center", background: "#0d0d0d", borderRadius: 8, padding: "10px 12px", flexWrap: "wrap" }}>
          <input value={newLabel} onChange={e => setNewLabel(e.target.value)} placeholder="Nombre" onKeyDown={e => e.key === "Enter" && createTag()} style={{ flex: 1, minWidth: 100, border: "1.5px solid #2a2a2a", borderRadius: 6, padding: "6px 10px", fontSize: 13, fontFamily: "inherit", outline: "none", background: "#111111" }} />
          <div style={{ display: "flex", gap: 4 }}>{TAG_PALETTE.map(c => <button key={c} onClick={() => setNewColor(c)} style={{ width: 18, height: 18, borderRadius: "50%", background: c, border: newColor === c ? "2.5px solid #1a1a1a" : "2px solid #fff", cursor: "pointer", boxShadow: "0 0 0 1px #333" }} />)}</div>
          <Btn onClick={createTag} style={{ padding: "6px 12px", fontSize: 12 }}>Crear</Btn>
          <button onClick={() => setAdding(false)} style={{ background: "none", border: "none", cursor: "pointer", color: "#555" }}><Icon name="x" size={14} /></button>
        </div>
      ) : (
        <button onClick={() => setAdding(true)} style={{ display: "flex", alignItems: "center", gap: 4, background: "none", border: "1.5px dashed #ddd", borderRadius: 20, padding: "3px 12px", fontSize: 12, color: "#555", cursor: "pointer", fontFamily: "inherit" }}><Icon name="plus" size={12} /> Nueva etiqueta</button>
      )}
    </div>
  );
};
    ...
    <>
      <Modal title={task ? "Editar tarea" : "Nueva tarea"} onClose={onClose} wide>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 16px" }}>
          ...
          {/* Multiple assignees */}
          <div style={{ marginBottom: 16 }}>
            <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#b0b0b0", marginBottom: 8, letterSpacing: "0.04em", textTransform: "uppercase" }}>Asignar a</label>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            {hoursPerPerson && form.assigneeIds.length > 1 && <p style={{ margin: "-8px 0 0", fontSize: 11, color: "#555" }}>~{hoursPerPerson}h por persona</p>}
          </div>
          ...
          {form.status === "blocked" && (
            <div style={{ gridColumn: "1/-1" }}>
              <Input label="¿Por qué está bloqueada?" placeholder="Ej: Esperando aprobación..." value={form.blockedReason} onChange={e => setForm({ ...form, blockedReason: e.target.value })} style={{ borderColor: "#e86c4a40", background: "#1f1210" }} />
            </div>
          )}
          ...
          <div style={{ gridColumn: "1/-1", marginBottom: 16 }}>
            <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#b0b0b0", marginBottom: 6, letterSpacing: "0.04em", textTransform: "uppercase" }}>Notas o link</label>
            <textarea value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} rows={2} placeholder="Comentario o link..." style={{ width: "100%", padding: "10px 12px", border: "1.5px solid #2a2a2a", borderRadius: 8, fontSize: 14, fontFamily: "inherit", outline: "none", resize: "vertical", boxSizing: "border-box", background: "#1e1e1e" }} />
          ...
          <div style={{ gridColumn: "1/-1" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
              <label style={{ fontSize: 12, fontWeight: 600, color: "#b0b0b0", letterSpacing: "0.04em", textTransform: "uppercase" }}>
                Subtareas {form.subtasks.length > 0 && <span style={{ color: "#555", fontWeight: 400 }}>({doneSubtasks}/{form.subtasks.length})</span>}
              </label>
              <button onClick={() => { setEditingSubtask(null); setShowSubtaskModal(true); }} style={{ display: "flex", alignItems: "center", gap: 4, background: "none", border: "1.5px dashed #ddd", borderRadius: 20, padding: "3px 10px", fontSize: 12, color: "#555", cursor: "pointer", fontFamily: "inherit" }}><Icon name="plus" size={12} /> Agregar</button>
            </div>
            {form.subtasks.length > 0 && (
              <div style={{ background: "#1e1e1e", borderRadius: 10, overflow: "hidden", marginBottom: 8 }}>
                {form.subtasks.map(st => {
                  const assignee = users.find(u => u.id === st.assigneeId);
                  const s = statusMap[st.status] || statusMap.todo;
                  return (
                    <div key={st.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 14px", borderBottom: "1px solid #222222" }}>
                      ...
                      <span style={{ fontSize: 11, color: "#444" }}>📅 {st.dueDate}</span>
                      {assignee && <Avatar user={assignee} size={20} />}
                      <button onClick={() => { setEditingSubtask(st); setShowSubtaskModal(true); }} style={{ background: "none", border: "none", cursor: "pointer", color: "#3a3a3a", padding: 2 }}><Icon name="edit" size={12} /></button>
    ...
    <div style={{ borderBottom: "1px solid #1a1a1a" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "13px 18px" }}>
        ...
        <select value={task.status} onChange={e => onStatusChange(task.id, e.target.value)}
          style={{ fontSize: 12, border: "1.5px solid #222222", borderRadius: 6, padding: "4px 8px", background: "#1e1e1e", cursor: "pointer", fontFamily: "inherit" }}>
          <option value="todo">Por hacer</option><option value="doing">En progreso</option><option value="blocked">Bloqueada</option><option value="done">Completado</option>
        </select>
        <button onClick={() => onEdit(task)} style={{ background: "none", border: "none", cursor: "pointer", color: "#3a3a3a", padding: 4 }}><Icon name="edit" size={14} /></button>
...
const ActivityFeed = ({ events, users, clients, tasks, limit }) => {
  ...
  if (shown.length === 0) return <p style={{ color: "#555", fontSize: 13, padding: "12px 0" }}>Sin actividad registrada aún.</p>;
  return (
    <div>
      {shown.map(ev => {
        const user = users.find(u => u.id === ev.userId);
        return (
          <div key={ev.id} style={{ display: "flex", gap: 10, alignItems: "flex-start", padding: "10px 0", borderBottom: "1px solid #1e1e1e" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 11, color: "#444", marginTop: 2 }}>{fmtTime(ev.timestamp)}</div>
            </div>
          </div>
        );
      })}
...
const MeetingCard = ({ meeting, clients, users, onEdit, onDelete, onConvertAction, onToggleAction, compact }) => {
  ...
  return (
    <div style={{ background: "#111111", borderRadius: 12, border: "1.5px solid #222222", overflow: "hidden" }}>
      ...
      <div style={{ padding: compact ? "14px 16px" : "18px 20px" }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10, marginBottom: 8 }}>
          ...
          {!compact && (
            <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
              <button onClick={() => onEdit(meeting)} style={{ background: "none", border: "none", cursor: "pointer", color: "#3a3a3a", padding: 3 }}><Icon name="edit" size={13} /></button>
        ...
        {meeting.body && (
          <p style={{ margin: "8px 0", fontSize: 13, color: "#b0b0b0", lineHeight: 1.6, display: expanded ? "block" : "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{meeting.body}</p>
        )}
        {!compact && actionItems.length > 0 && (
          <div style={{ marginTop: 12 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: "#555", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 8 }}>Action items</div>
...
const ConvertActionModal = ({ item, clients, users, meeting, onSave, onClose }) => {
  ...
  return (
    <Modal title="Convertir en tarea" onClose={onClose}>
      <p style={{ fontSize: 13, color: "#707070", marginTop: 0 }}>Se creará una tarea a partir de este action item.</p>
...
const MeetingModal = ({ meeting, clients, users, onSave, onClose }) => {
  ...
  return (
    <Modal title={meeting ? "Editar reunión" : "Nueva reunión"} onClose={onClose} wide>
      ...
      <div style={{ marginBottom: 16 }}>
        <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#b0b0b0", marginBottom: 8, letterSpacing: "0.04em", textTransform: "uppercase" }}>Participantes</label>
      ...
      <div style={{ marginBottom: 20 }}>
        <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#b0b0b0", marginBottom: 8, letterSpacing: "0.04em", textTransform: "uppercase" }}>Action items</label>
        {form.actionItems.map((item, i) => (
          <div key={i} style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8 }}>
            <div style={{ width: 16, height: 16, borderRadius: 4, border: "1.5px solid #2a2a2a", flexShrink: 0 }} />
            <input value={item.text} onChange={e => updateAction(i, e.target.value)} placeholder={`Acción ${i + 1}...`}
              style={{ flex: 1, border: "1.5px solid #2a2a2a", borderRadius: 8, padding: "8px 12px", fontSize: 13, fontFamily: "inherit", outline: "none", background: "#1e1e1e" }}
              onFocus={e => e.target.style.borderColor = "#5b6af0"} onBlur={e => e.target.style.borderColor = "#2a2a2a"} />
            {form.actionItems.length > 1 && <button onClick={() => setForm(f => ({ ...f, actionItems: f.actionItems.filter((_, idx) => idx !== i) }))} style={{ background: "none", border: "none", cursor: "pointer", color: "#333", padding: 2 }}><Icon name="x" size={14} /></button>}
          </div>
        ))}
        <button onClick={() => setForm(f => ({ ...f, actionItems: [...f.actionItems, { text: "", converted: false }] }))} style={{ display: "flex", alignItems: "center", gap: 4, background: "none", border: "1.5px dashed #ddd", borderRadius: 8, padding: "6px 14px", fontSize: 12, color: "#555", cursor: "pointer", fontFamily: "inherit", marginTop: 4 }}>
...
const LoginScreen = ({ users, onLogin }) => {
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#0d0d0d", fontFamily: "Lora, Georgia, serif" }}>
      <div style={{ width: 380, background: "#111111", borderRadius: 20, padding: "48px 40px", boxShadow: "0 8px 60px rgba(0,0,0,0.5)", border: "1.5px solid #222" }}>
        <div style={{ marginBottom: 36, textAlign: "center" }}>
          ...
          <h1 style={{ margin: "0 0 6px", fontSize: 26, fontWeight: 700, letterSpacing: "-0.02em", color: "#f0f0f0" }}>TeamFlow</h1>
          <p style={{ margin: 0, fontSize: 13, color: "#707070", fontFamily: "'DM Sans', sans-serif" }}>Workspace para equipos creativos</p>
        </div>
        ...
        <div style={{ marginTop: 24, padding: 16, background: "#0d0d0d", borderRadius: 10 }}>
          <p style={{ margin: "0 0 8px", fontSize: 11, fontWeight: 600, color: "#707070", letterSpacing: "0.05em", textTransform: "uppercase", fontFamily: "'DM Sans', sans-serif" }}>Cuentas de prueba</p>
          {users.map(u => <p key={u.id} style={{ margin: "3px 0", fontSize: 12, color: "#b0b0b0", fontFamily: "'DM Sans', sans-serif" }}>{u.email} / {u.password}</p>)}
        </div>
      </div>
    </div>
  );
...
const Dashboard = ({ tasks, setTasks, clients, users, tags, setTags, meetings, activity, setActiveSection, currentUser }) => {
  ...
  const getDaysLabel = (d) => {
    ...
    if (diff < 0)   return { text: `Venció hace ${Math.abs(diff)}d`, color: "#e86c4a", bg: "#2a1008" };
    if (diff === 0) return { text: "Vence hoy",    color: "#e86c4a", bg: "#2a1008" };
    if (diff === 1) return { text: "Vence mañana", color: "#f0a030", bg: "#251a00" };
    if (diff <= 7)  return { text: `En ${diff}d`,  color: "#f0a030", bg: "#251a00" };
    return { text: `En ${diff}d`, color: "#555", bg: "#1a1a1a" };
  };

  return (
    <div>
      <h2 style={{ margin: "0 0 6px", fontSize: 24, fontWeight: 700, letterSpacing: "-0.02em" }}>Buen día, {currentUser.name} 👋</h2>
      <p style={{ margin: "0 0 28px", color: "#707070", fontSize: 14 }}>Aquí está el resumen del equipo.</p>
      ...
      <div className="tf-stats">
        {[
          ...
          <div key={s.label} style={{ background: "#111111", borderRadius: 14, padding: "18px 20px", border: "1.5px solid #222222" }}>
            <div style={{ fontSize: 26, fontWeight: 700, color: s.color, marginBottom: 4 }}>{s.value}</div>
            <div style={{ fontSize: 13, color: "#707070" }}>{s.label}</div>
          </div>
        ))}
      </div>
      ...
      <div className="tf-2col">
        {/* Tasks */}
        <div style={{ background: "#111111", borderRadius: 14, border: "1.5px solid #222222", overflow: "hidden" }}>
          <div style={{ padding: "18px 20px 12px", borderBottom: "1.5px solid #1e1e1e" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
              <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700 }}>Tareas por vencer</h3>
              <span style={{ fontSize: 12, color: "#555" }}>{pendingTasks.length} pendientes</span>
            </div>
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          ...
          {pendingTasks.length === 0 && <p style={{ color: "#555", fontSize: 13, padding: 20 }}>¡Sin tareas pendientes! 🎉</p>}
          {pendingTasks.slice(0, 6).map(t => {
            ...
            return (
              <div key={t.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 20px", borderBottom: "1px solid #1a1a1a", background: isBlocked ? "#fff8f7" : "transparent" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: "flex", gap: 5, flexWrap: "wrap", alignItems: "center" }}>
                    ...
                    {taskTags.map(tag => <TagChip key={tag.id} tag={tag} />)}
                    {sub.length > 0 && <span style={{ fontSize: 11, color: "#555" }}>⬜ {doneSub}/{sub.length}</span>}
                  </div>
                </div>
                ...
                <button onClick={() => setEditingTask(t)} style={{ background: "none", border: "none", cursor: "pointer", color: "#333", padding: 2 }}><Icon name="edit" size={13} /></button>
              </div>
            );
          })}
        </div>
        ...
        {/* Team progress */}
        <div style={{ background: "#111111", borderRadius: 14, padding: 22, border: "1.5px solid #222222" }}>
          <h3 style={{ margin: "0 0 16px", fontSize: 15, fontWeight: 700 }}>Progreso del equipo</h3>
          <div style={{ marginBottom: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6, fontSize: 13, color: "#b0b0b0" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 12, fontWeight: 700, display: "flex", justifyContent: "space-between" }}>
                      <span>{u.name}</span><span style={{ color: "#555", fontWeight: 500 }}>{uDone}/{uTasks.length}</span>
                    </div>
      ...
      {(() => {
        ...
        return (
          <div style={{ marginTop: 20, background: "#111111", borderRadius: 14, border: "1.5px solid #222222", overflow: "hidden" }}>
            <div style={{ padding: "16px 22px", borderBottom: "1.5px solid #1e1e1e" }}>
              <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700 }}>Carga esta semana</h3>
              <p style={{ margin: "2px 0 0", fontSize: 12, color: "#555" }}>{fmt(monday)} – {fmt(sunday)} · {wt.length} tareas · {total}h totales</p>
            </div>
                ...
                return (
                  <div key={u.id} style={{ padding: "14px 20px", borderRight: "1px solid #f5f5f5" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5, fontSize: 12 }}>
                      <span style={{ color: "#555" }}>{uC} tarea{uC !== 1 ? "s" : ""}</span>
      ...
      <div className="tf-bottom-row">
        ...
        <div>
          ...
          <div style={{ background: "#111111", borderRadius: 14, padding: "4px 18px", border: "1.5px solid #222222" }}>
...
const ActivityView = ({ activity, users, clients, tasks }) => {
  ...
  return (
    <div>
      ...
      <p style={{ margin: "0 0 24px", color: "#707070", fontSize: 14 }}>Historial completo de cambios del equipo.</p>
      <div style={{ background: "#111111", borderRadius: 14, border: "1.5px solid #222222", padding: "8px 24px" }}>
...
const ClientsView = ({ clients, setClients, tasks, pages, meetings, setActiveSection, setSelectedClient }) => {
  ...
  const addClient = () => {
    ...
    setClients([...clients, { id: "c" + Date.now(), name: form.name, industry: form.industry, color: colors[clients.length % colors.length], initials, logo: null, status: form.status, since: parseInt(form.since), createdAt: Date.now() }]);
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        <div><h2 style={{ margin: "0 0 4px", fontSize: 24, fontWeight: 700, letterSpacing: "-0.02em" }}>Clientes</h2><p style={{ margin: 0, color: "#707070", fontSize: 14 }}>{clients.length} clientes en total</p></div>
        ...
        {filtered.map(c => {
          ...
          return (
            <div key={c.id} onClick={() => { setSelectedClient(c); setActiveSection("client-detail"); }}
              style={{ background: "#111111", borderRadius: 14, border: "1.5px solid #222222", overflow: "hidden", cursor: "pointer", transition: "box-shadow 0.2s", opacity: c.status === "closed" ? 0.65 : 1 }}
              ...
              <div style={{ padding: 22 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 12, color: "#555" }}>{c.industry}</div>
                  </div>
                  <button onClick={e => { e.stopPropagation(); if (window.confirm(`¿Borrar "${c.name}" y todos sus datos? Esta acción no se puede deshacer.`)) setClients(p => p.filter(x => x.id !== c.id)); }}
                    style={{ background: "none", border: "none", cursor: "pointer", color: "#333", padding: 4, flexShrink: 0 }} title="Borrar cliente">
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 10, flexWrap: "wrap" }}>
                  ...
                  {c.since && <span style={{ fontSize: 11, color: "#444" }}>desde {c.since}</span>}
                </div>
                <div style={{ display: "flex", gap: 10, fontSize: 12, color: "#707070", flexWrap: "wrap" }}>
...
const ClientDetail = ({ client, clients, setClients, tasks, setTasks, pages, setPages, meetings, setMeetings, tags, setTags, users, currentUser, onBack, addActivity }) => {
  ...
  return (
    <div>
      <button onClick={onBack} style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", cursor: "pointer", color: "#707070", fontSize: 13, marginBottom: 20, padding: 0, fontFamily: "inherit" }}>← Volver a clientes</button>
      <div style={{ display: "flex", alignItems: "flex-start", gap: 20, marginBottom: 28 }}>
        ...
        <div style={{ flex: 1, paddingTop: 4 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
            ...
            {liveClient.since && <span style={{ fontSize: 12, color: "#444" }}>desde {liveClient.since}</span>}
          </div>
          <p style={{ margin: "0 0 10px", color: "#555", fontSize: 13 }}>{liveClient.industry}</p>
          <button onClick={() => { setClientForm({ status: liveClient.status || "active", since: liveClient.since || new Date().getFullYear() }); setShowEditClient(true); }}
            style={{ display: "flex", alignItems: "center", gap: 5, background: "none", border: "1.5px solid #222222", borderRadius: 8, padding: "5px 12px", fontSize: 12, color: "#707070", cursor: "pointer", fontFamily: "inherit" }}>
          ...
          {["todo","doing","blocked","done"].map(status => {
            ...
            return (
              <div key={status} style={{ marginBottom: 24 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}><div style={{ width: 8, height: 8, borderRadius: "50%", background: s.color }} /><span style={{ fontSize: 12, fontWeight: 700, color: s.color, textTransform: "uppercase", letterSpacing: "0.06em" }}>{s.label}</span><span style={{ fontSize: 11, color: "#3a3a3a" }}>{group.length}</span></div>
                <div style={{ background: "#111111", borderRadius: 10, border: "1.5px solid #222222", overflow: "hidden" }}>
      ...
      {tab === "meetings" && (
        <div>
          ...
          {cMeetings.length === 0 && <p style={{ color: "#555", fontSize: 14 }}>Sin reuniones registradas.</p>}
      ...
      {tab === "pages" && (
        <div>
          ...
          {cPages.length === 0 && <p style={{ color: "#555", fontSize: 14 }}>Sin páginas aún.</p>}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 16 }}>
            {[...cPages].sort((a,b) => (b.date||"").localeCompare(a.date||"")).map(p => {
              ...
              return (
                <div key={p.id} style={{ background: "#111111", borderRadius: 14, border: "1.5px solid #222222", overflow: "hidden", display: "flex", flexDirection: "column" }}>
                  ...
                  <div style={{ padding: "16px 18px", flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 10 }}>
                      ...
                      <div style={{ display: "flex", gap: 2, flexShrink: 0, marginLeft: 8 }}>
                        <button onClick={() => setPreviewPage(p)} title="Vista previa" style={{ background: "none", border: "none", cursor: "pointer", color: "#3a3a3a", padding: 3 }}><Icon name="eye" size={13} /></button>
                        <button onClick={() => { setEditingPage(p); setPageForm({ title: p.title, content: p.content, type: p.type || "post", authorId: p.authorId || currentUser.id, date: p.date || new Date().toISOString().split("T")[0], notes: p.notes || [] }); setShowPageModal(true); }} style={{ background: "none", border: "none", cursor: "pointer", color: "#3a3a3a", padding: 3 }}><Icon name="edit" size={13} /></button>
                    ...
                    {p.content && <p style={{ margin: "0 0 10px", fontSize: 12, color: "#808080", lineHeight: 1.6, display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{p.content}</p>}
                    {notes.length > 0 && (
                      <div style={{ marginBottom: 10 }}>
                        <div style={{ fontSize: 10, fontWeight: 700, color: "#444", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 5 }}>Notas {checked}/{notes.length}</div>
                        {notes.map((n, i) => (
                  ...
                  <div style={{ padding: "10px 18px", borderTop: "1px solid #1e1e1e", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      {author && <><Avatar user={author} size={18} /><span style={{ fontSize: 11, color: "#555" }}>{author.name}</span></>}
                    </div>
                    {p.date && <span style={{ fontSize: 11, color: "#444" }}>📅 {p.date}</span>}
                  </div>
                </div>
              );
            })}
          ...
          <Modal title="" onClose={() => setPreviewPage(null)} wide>
            ...
            <div style={{ marginBottom: 20 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                ...
                {p.date && <span style={{ fontSize: 12, color: "#444" }}>📅 {p.date}</span>}
                {author && <span style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 12, color: "#555" }}><Avatar user={author} size={18} />{author.name}</span>}
              </div>
            ...
            {/* Content */}
            {p.content && (
              <div style={{ marginBottom: 20, padding: "16px 20px", background: "#1a1a1a", borderRadius: 10, fontSize: 14, lineHeight: 1.8, color: "#444", whiteSpace: "pre-wrap" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 16px" }}>
            ...
            <div style={{ gridColumn: "1/-1" }}>
              ...
              {pageForm.content && /(https?:\/\/[^\s]+)/.test(pageForm.content) && (
                <div style={{ marginTop: -10, marginBottom: 16, fontSize: 12, color: "#555" }}>
            ...
            <div style={{ gridColumn: "1/-1" }}>
              <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#b0b0b0", marginBottom: 8, letterSpacing: "0.04em", textTransform: "uppercase" }}>
                Action notes {pageForm.notes?.length > 0 && <span style={{ color: "#444", fontWeight: 400 }}>({pageForm.notes.filter(n=>n.checked).length}/{pageForm.notes.length})</span>}
              </label>
              {(pageForm.notes || []).map((n, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                  ...
                  <input value={n.text} onChange={e => setPageForm(f => ({ ...f, notes: f.notes.map((nn, ii) => ii === i ? { ...nn, text: e.target.value } : nn) }))}
                    style={{ flex: 1, border: "1.5px solid #2a2a2a", borderRadius: 6, padding: "6px 10px", fontSize: 13, fontFamily: "inherit", outline: "none", background: "#1e1e1e", textDecoration: n.checked ? "line-through" : "none", color: n.checked ? "#aaa" : "#1a1a1a" }} />
                  <button onClick={() => setPageForm(f => ({ ...f, notes: f.notes.filter((_, ii) => ii !== i) }))} style={{ background: "none", border: "none", cursor: "pointer", color: "#333", padding: 2 }}><Icon name="x" size={13} /></button>
                </div>
              ))}
              <button onClick={() => setPageForm(f => ({ ...f, notes: [...(f.notes||[]), { text: "", checked: false }] }))}
                style={{ display: "flex", alignItems: "center", gap: 4, background: "none", border: "1.5px dashed #ddd", borderRadius: 8, padding: "6px 14px", fontSize: 12, color: "#555", cursor: "pointer", fontFamily: "inherit", marginTop: 4 }}>
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        <div><h2 style={{ margin: "0 0 4px", fontSize: 24, fontWeight: 700, letterSpacing: "-0.02em" }}>Tareas</h2><p style={{ margin: 0, color: "#707070", fontSize: 14 }}>{tasks.length} tareas en total</p></div>
        <Btn onClick={() => { setEditingTask(null); setShowNewTask(true); }}><span style={{ display: "flex", alignItems: "center", gap: 6 }}><Icon name="plus" size={14} /> Nueva tarea</span></Btn>
      </div>
      ...
      {tags.length > 0 && (
        <div style={{ display: "flex", gap: 6, marginBottom: 20, flexWrap: "wrap", alignItems: "center" }}>
          <span style={{ fontSize: 11, color: "#444", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", marginRight: 2 }}>Etiqueta:</span>
          <button onClick={() => setTagFilter("all")} style={{ padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 600, border: "1.5px solid", borderColor: tagFilter === "all" ? "#1a1a1a" : "#e8e8e8", background: tagFilter === "all" ? "#1a1a1a" : "transparent", color: tagFilter === "all" ? "#fff" : "#888", cursor: "pointer", fontFamily: "inherit" }}>Todas</button>
          {tags.map(tag => <button key={tag.id} onClick={() => setTagFilter(tagFilter === tag.id ? "all" : tag.id)} style={{ padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 600, border: `1.5px solid ${tag.color}`, background: tagFilter === tag.id ? tag.color : "transparent", color: tagFilter === tag.id ? "#fff" : tag.color, cursor: "pointer", fontFamily: "inherit", transition: "all 0.15s" }}>{tag.label}</button>)}
        </div>
      )}
      ...
      {/* Active tasks */}
      {!showingDone && (
        <div style={{ background: "#111111", borderRadius: 14, border: "1.5px solid #222222", overflow: "hidden", marginBottom: doneTasks.length > 0 ? 12 : 0 }}>
          {filtered.length === 0 && <p style={{ padding: 24, color: "#555", fontSize: 14 }}>Sin tareas activas para este filtro.</p>}
      ...
      {!showingDone && doneTasks.length > 0 && (
        <div style={{ background: "#111111", borderRadius: 14, border: "1.5px solid #222222", overflow: "hidden" }}>
          <button onClick={() => setShowDone(s => !s)} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px", background: "none", border: "none", cursor: "pointer", fontFamily: "inherit" }}>
            <span style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, fontWeight: 700, color: "#555" }}>
              <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#3db88a" }} />
              Completadas <span style={{ fontWeight: 400 }}>({doneTasks.length})</span>
            </span>
            <span style={{ fontSize: 11, color: "#444" }}>{showDone ? "▲ ocultar" : "▼ mostrar"}</span>
          </button>
      ...
      {showingDone && (
        <div style={{ background: "#111111", borderRadius: 14, border: "1.5px solid #222222", overflow: "hidden" }}>
          {doneTasks.length === 0 && <p style={{ padding: 24, color: "#555", fontSize: 14 }}>Sin tareas completadas.</p>}
...
const MeetingsView = ({ meetings, setMeetings, clients, users, tasks, setTasks, currentUser, addActivity }) => {
  ...
  const sorted   = [...filtered].sort((a, b) => b.date.localeCompare(a.date) || (b.createdAt || 0) - (a.createdAt || 0));
  const saveMeeting = (form) => {
    if (editingMeeting) setMeetings(p => p.map(m => m.id === editingMeeting.id ? { ...m, ...form } : m));
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        <div><h2 style={{ margin: "0 0 4px", fontSize: 24, fontWeight: 700, letterSpacing: "-0.02em" }}>Meeting Notes</h2><p style={{ margin: 0, color: "#707070", fontSize: 14 }}>{meetings.length} reuniones registradas</p></div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 24, flexWrap: "wrap" }}>
        ...
        {[...clients].sort((a,b) => (a.createdAt||a.id||"").localeCompare(b.createdAt||b.id||"")).map(c => <button key={c.id} onClick={() => setClientFilter(clientFilter === c.id ? "all" : c.id)} style={{ padding: "6px 14px", borderRadius: 8, border: `1.5px solid ${clientFilter === c.id ? c.color : "#e8e8e8"}`, background: clientFilter === c.id ? c.color + "15" : "#fff", color: clientFilter === c.id ? c.color : "#555", fontSize: 13, cursor: "pointer", fontFamily: "inherit", fontWeight: clientFilter === c.id ? 700 : 500, transition: "all 0.15s" }}>{c.name}</button>)}
      </div>
      {sorted.length === 0 && <p style={{ color: "#555", fontSize: 14 }}>Sin reuniones registradas.</p>}
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
        <div><h2 style={{ margin: "0 0 4px", fontSize: 24, fontWeight: 700, letterSpacing: "-0.02em" }}>Carga del equipo</h2><p style={{ margin: 0, color: "#707070", fontSize: 14 }}>Horas estimadas por semana</p></div>
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <button onClick={() => setWeekOffset(w => w - 1)} style={{ background: "#111111", border: "1.5px solid #222222", borderRadius: 8, padding: "7px 10px", cursor: "pointer", display: "flex" }}><Icon name="chevL" size={16} /></button>
            <span style={{ fontSize: 13, fontWeight: 600, minWidth: 140, textAlign: "center" }}>{fmt(monday)} – {fmt(sunday)}</span>
            <button onClick={() => setWeekOffset(w => w + 1)} style={{ background: "#111111", border: "1.5px solid #222222", borderRadius: 8, padding: "7px 10px", cursor: "pointer", display: "flex" }}><Icon name="chevR" size={16} /></button>
      ...
      <div className="tf-workload-cards">
        {userStats.map(({ user, tasks: uT, completedTasks: uC, totalHours, totalAllHours }) => {
          ...
          return (
            <div key={user.id} style={{ background: "#111111", borderRadius: 14, padding: "18px 20px", border: "1.5px solid #222222" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}><Avatar user={user} size={32} /><div><div style={{ fontSize: 13, fontWeight: 700 }}>{user.name}</div><div style={{ fontSize: 11, color: "#555" }}>{uT.length} activas{showCompleted && uC.length > 0 ? ` · ${uC.length} completadas` : ""}</div></div></div>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6, fontSize: 12 }}>
                <span style={{ color: "#707070" }}>Carga activa</span>
                <span style={{ fontWeight: 700, color: col }}>{totalHours}h{showCompleted && uC.length > 0 ? <span style={{ color: "#444", fontWeight: 400 }}> +{(totalAllHours - totalHours).toFixed(1)}h ✓</span> : ""}</span>
              </div>
              ...
              <div style={{ fontSize: 11, color: "#555" }}>~{(totalHours / 5).toFixed(1)}h/día estimado</div>
            </div>
          );
        })}
      </div>
      ...
      <div style={{ background: "#111111", borderRadius: 14, border: "1.5px solid #222222", overflow: "hidden" }}>
        <div style={{ padding: "18px 22px 14px", borderBottom: "1.5px solid #1e1e1e" }}>
          <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700 }}>Distribución diaria</h3>
          <p style={{ margin: "3px 0 0", fontSize: 12, color: "#555" }}>Las horas se distribuyen entre la fecha de inicio y el deadline de cada tarea</p>
        </div>
        <div className="tf-workload-heatmap" style={{ padding: "10px 22px 0", borderBottom: "1px solid #1e1e1e" }}>
          <div />
        ...
        {userStats.map(({ user, byDay }) => (
          <div key={user.id} className="tf-workload-heatmap" style={{ padding: "10px 22px", borderBottom: "1px solid #1a1a1a", alignItems: "center" }}>
                ...
                <div key={dl.date} style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "4px 2px" }}>
                  {dl.hours > 0
                    ? <div title={dl.tasks.map(t => `${t.title} (${getHoursForDay(t, dl.date)}h)`).join("\n")}
                        ...
                        {hasDone && <span style={{ fontSize: 9, color: "#444", fontWeight: 400 }}>+{dl.completedH}✓</span>}
                      </div>
                    : <div style={{ width: 36, height: 36, borderRadius: 8, background: "#1a1a1a", display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ fontSize: 10, color: "#333" }}>—</span></div>}
                </div>
              );
            })}
          </div>
        ))}
        <div style={{ display: "flex", gap: 16, padding: "12px 22px", borderTop: "1px solid #1e1e1e", alignItems: "center", flexWrap: "wrap" }}>
          <span style={{ fontSize: 11, color: "#555", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em" }}>Carga activa:</span>
          {[["#3db88a","≤24h"],["#f0a030","25–35h"],["#e86c4a",">35h"]].map(([c,l]) => <div key={c} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 11, color: "#707070" }}><div style={{ width: 12, height: 12, borderRadius: 3, background: c }} />{l}</div>)}
          {showCompleted && <span style={{ fontSize: 11, color: "#444", marginLeft: 8 }}>· Las completadas aparecen en gris</span>}
        </div>
      </div>
    </div>
  );
...
const CalendarView = ({ tasks, clients }) => {
  ...
  return (
    <div>
      ...
      <div style={{ background: "#111111", borderRadius: 16, border: "1.5px solid #222222", overflow: "hidden" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 24px", borderBottom: "1.5px solid #1e1e1e" }}>
          <button onClick={() => setCurrent(c => c.month === 0 ? { year: c.year-1, month: 11 } : { ...c, month: c.month-1 })} style={{ background: "none", border: "none", cursor: "pointer", color: "#707070", padding: 8, fontSize: 20 }}>‹</button>
          <span style={{ fontSize: 16, fontWeight: 700 }}>{mN[current.month]} {current.year}</span>
          <button onClick={() => setCurrent(c => c.month === 11 ? { year: c.year+1, month: 0 } : { ...c, month: c.month+1 })} style={{ background: "none", border: "none", cursor: "pointer", color: "#707070", padding: 8, fontSize: 20 }}>›</button>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)" }}>
          {dN.map(d => <div key={d} style={{ padding: "10px 4px", textAlign: "center", fontSize: 11, fontWeight: 700, color: "#444", letterSpacing: "0.05em", borderBottom: "1px solid #1e1e1e" }}>{d}</div>)}
          {cells.map((day, i) => {
            ...
            const iT = ds === ts;
            return (
              <div key={day} style={{ minHeight: 80, padding: "8px 6px", borderRight: "1px solid #1e1e1e", borderBottom: "1px solid #1a1a1a", background: iT ? "#1e2035" : "transparent" }}>
                <div style={{ width: 26, height: 26, borderRadius: "50%", background: iT ? "#5b6af0" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: iT ? 700 : 500, color: iT ? "#fff" : "#707070", marginBottom: 4 }}>{day}</div>
...
// ─── Settings View ────────────────────────────────────────────────────────────
const SettingsView = ({ users, setUsers, clients, setClients, currentUser }) => {
  const [editingUser, setEditingUser] = useState(null);
  const [form, setForm]               = useState(null);
  const openEdit  = (u) => { setEditingUser(u); setForm({ role: u.role || "member", allowedClients: u.allowedClients || [] }); };
  const saveUser  = () => { setUsers(prev => prev.map(u => u.id === editingUser.id ? { ...u, ...form } : u)); setEditingUser(null); };
  const toggleC   = (cid) => setForm(f => ({ ...f, allowedClients: f.allowedClients.includes(cid) ? f.allowedClients.filter(x => x !== cid) : [...f.allowedClients, cid] }));
  const genToken  = (cid) => { const tok = Math.random().toString(36).slice(2,10)+Math.random().toString(36).slice(2,10); setClients(prev => prev.map(c => c.id === cid ? { ...c, publicToken: tok } : c)); };
  const revokeT   = (cid) => setClients(prev => prev.map(c => c.id === cid ? { ...c, publicToken: null } : c));
  const base      = window.location.origin + window.location.pathname;
  return (
    <div>
      <h2 style={{ margin: "0 0 6px", fontSize: 24, fontWeight: 700, letterSpacing: "-0.02em" }}>Ajustes</h2>
      <p style={{ margin: "0 0 28px", color: "#707070", fontSize: 14 }}>Solo visible para administradores.</p>
      <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 14 }}>Usuarios y permisos</h3>
      <div style={{ background: "#111111", borderRadius: 14, border: "1.5px solid #222222", overflow: "hidden", marginBottom: 32 }}>
        {users.map((u, i) => (
          <div key={u.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 20px", borderBottom: i < users.length-1 ? "1px solid #f5f5f5" : "none" }}>
            <Avatar user={u} size={36} />
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 14, fontWeight: 700 }}>{u.name}</div>
              <div style={{ fontSize: 12, color: "#555" }}>{u.email}</div>
              {u.role !== "admin" && (u.allowedClients||[]).length > 0 && <div style={{ fontSize: 11, color: "#707070", marginTop: 3 }}>Clientes: {(u.allowedClients||[]).map(cid => clients.find(c=>c.id===cid)?.name).filter(Boolean).join(", ")}</div>}
              {u.role !== "admin" && !(u.allowedClients||[]).length && <div style={{ fontSize: 11, color: "#3db88a", marginTop: 3 }}>Acceso a todos los clientes</div>}
            </div>
            <span style={{ fontSize: 11, fontWeight: 700, color: u.role === "admin" ? "#5b6af0" : "#888", background: u.role === "admin" ? "#5b6af010" : "#f5f5f5", padding: "3px 10px", borderRadius: 20, textTransform: "uppercase", letterSpacing: "0.04em" }}>{u.role || "member"}</span>
            {u.id !== currentUser.id && <button onClick={() => openEdit(u)} style={{ background: "none", border: "1.5px solid #2a2a2a", borderRadius: 8, padding: "6px 12px", cursor: "pointer", fontSize: 12, fontFamily: "inherit", color: "#b0b0b0" }}>Editar</button>}
          </div>
        ))}
      </div>
      <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 6 }}>Links públicos por cliente</h3>
      <p style={{ fontSize: 13, color: "#555", marginBottom: 14 }}>Compartí las tareas activas y el contenido de un cliente sin necesidad de login.</p>
      <div style={{ background: "#111111", borderRadius: 14, border: "1.5px solid #222222", overflow: "hidden" }}>
        {clients.map((c, i) => {
          const link = c.publicToken ? `${base}?client=${c.publicToken}` : null;
          return (
            <div key={c.id} style={{ padding: "14px 20px", borderBottom: i < clients.length-1 ? "1px solid #f5f5f5" : "none" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: link ? 10 : 0 }}>
                <div style={{ width: 10, height: 10, borderRadius: "50%", background: c.color }} />
                <span style={{ fontWeight: 600, fontSize: 14, flex: 1 }}>{c.name}</span>
                {link
                  ? <button onClick={() => revokeT(c.id)} style={{ background: "none", border: "1.5px solid #e86c4a30", borderRadius: 8, padding: "6px 12px", cursor: "pointer", fontSize: 12, fontFamily: "inherit", color: "#e86c4a" }}>Revocar</button>
                  : <button onClick={() => genToken(c.id)} style={{ background: "#5b6af0", border: "none", borderRadius: 8, padding: "6px 12px", cursor: "pointer", fontSize: 12, fontFamily: "inherit", color: "#fff", fontWeight: 600 }}>Generar link</button>}
              </div>
              {link && (
                <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#0d0d0d", borderRadius: 8, padding: "8px 12px" }}>
                  <span style={{ fontSize: 12, color: "#b0b0b0", flex: 1, wordBreak: "break-all" }}>{link}</span>
                  <button onClick={() => navigator.clipboard.writeText(link)} style={{ background: "none", border: "1.5px solid #2a2a2a", borderRadius: 6, padding: "4px 10px", cursor: "pointer", fontSize: 11, fontFamily: "inherit", color: "#707070", whiteSpace: "nowrap" }}>Copiar</button>
                  <a href={link} target="_blank" rel="noreferrer" style={{ background: "none", border: "1.5px solid #2a2a2a", borderRadius: 6, padding: "4px 10px", fontSize: 11, fontFamily: "inherit", color: "#707070", textDecoration: "none", whiteSpace: "nowrap" }}>Abrir</a>
                </div>
              )}
            </div>
          );
      ...
      {editingUser && form && (
        <Modal title={`Editar: ${editingUser.name}`} onClose={() => setEditingUser(null)}>
          <div style={{ marginBottom: 16 }}>
            <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#b0b0b0", marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.04em" }}>Rol</label>
            <div style={{ display: "flex", gap: 8 }}>
              {["admin","member"].map(r => <button key={r} onClick={() => setForm(f => ({ ...f, role: r }))} style={{ flex: 1, padding: "9px 0", borderRadius: 8, border: `1.5px solid ${form.role === r ? "#5b6af0" : "#e8e8e8"}`, background: form.role === r ? "#5b6af010" : "#fff", color: form.role === r ? "#5b6af0" : "#888", fontWeight: form.role === r ? 700 : 500, fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>{r === "admin" ? "Administrador" : "Miembro"}</button>)}
            </div>
            <p style={{ margin: "8px 0 0", fontSize: 11, color: "#555" }}>{form.role === "admin" ? "Ve y edita todo. Accede a Ajustes." : "Ve solo los clientes que le asignés."}</p>
          </div>
          {form.role === "member" && (
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#b0b0b0", marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.04em" }}>Clientes visibles</label>
              <p style={{ margin: "0 0 10px", fontSize: 12, color: "#555" }}>Sin selección = ve todos los clientes.</p>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {clients.map(c => { const active = (form.allowedClients||[]).includes(c.id); return (
                  <button key={c.id} onClick={() => toggleC(c.id)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderRadius: 10, border: `1.5px solid ${active ? c.color : "#e8e8e8"}`, background: active ? c.color+"10" : "#fafafa", cursor: "pointer", fontFamily: "inherit", textAlign: "left" }}>
                    <div style={{ width: 10, height: 10, borderRadius: "50%", background: c.color }} />
                    <span style={{ fontSize: 13, fontWeight: active ? 700 : 500, color: active ? c.color : "#555", flex: 1 }}>{c.name}</span>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
            <Btn variant="secondary" onClick={() => setEditingUser(null)}>Cancelar</Btn>
            <Btn onClick={saveUser}>Guardar</Btn>
          </div>
        </Modal>
      )}
    </div>
...
// ─── Sidebar Content ──────────────────────────────────────────────────────────
const SidebarContent = ({ navItems, activeSection, navigate, setSidebarOpen, currentUser, isAdmin }) => (
  <>
    <div style={{ padding: "24px 20px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <div style={{ width: 32, height: 32, background: "#1a1a1a", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 14, fontWeight: 700, fontFamily: "Lora, Georgia, serif" }}>T</div>
        <span style={{ fontWeight: 700, fontSize: 16, letterSpacing: "-0.02em", fontFamily: "Lora, Georgia, serif" }}>TeamFlow</span>
      </div>
      <button onClick={() => setSidebarOpen(false)} className="tf-hamburger-close" style={{ background: "none", border: "none", cursor: "pointer", color: "#707070", padding: 4, display: "none" }}><Icon name="x" size={18} /></button>
    </div>
    ...
    <div style={{ padding: "16px 14px", borderTop: "1.5px solid #1e1e1e" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
        <Avatar user={currentUser} size={32} />
        <div style={{ minWidth: 0 }}>
          <div style={{ fontSize: 13, fontWeight: 700 }}>{currentUser.name}</div>
          <div style={{ fontSize: 11, color: "#555", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{currentUser.email}</div>
          <div style={{ fontSize: 10, color: isAdmin ? "#5b6af0" : "#aaa", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em" }}>{isAdmin ? "Admin" : "Miembro"}</div>
        </div>
      </div>
      <button onClick={() => { localStorage.removeItem("tf-user"); window.location.reload(); }} style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "8px 10px", border: "none", background: "none", cursor: "pointer", color: "#555", fontSize: 12, fontFamily: "inherit", borderRadius: 6 }}>
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // Load users separately — don't block app if it fails
      try {
        const su = await storage.get("tf-users");
        if (su) {
          setUsers(SEED.users.map(seed => {
            const remote = su.find(u => u.id === seed.id);
            return remote ? { ...seed, ...remote } : seed;
          }));
        }
      } catch (e) { console.warn("tf-users load failed, using defaults", e); }
      setLoading(false);
    })();
  }, []);

  // Polling — sync changes from other users every 15s
  const localRef = useRef({ clients, tasks, pages, tags, meetings, activity, users });
  useEffect(() => { localRef.current = { clients, tasks, pages, tags, meetings, activity, users }; }, [clients, tasks, pages, tags, meetings, activity, users]);
  useEffect(() => {
    if (loading) return;
    const stable = (arr) => arr ? [...arr].sort((a,b) => String(a.id).localeCompare(String(b.id))) : arr;
    const changed = (remote, local) => remote && JSON.stringify(stable(remote)) !== JSON.stringify(stable(local));
    const id = setInterval(async () => {
      const [sc, st, sp, stg, sm, sa] = await Promise.all([
        storage.get("tf-clients"), storage.get("tf-tasks"), storage.get("tf-pages"),
        storage.get("tf-tags"),    storage.get("tf-meetings"), storage.get("tf-activity"),
      ]);
      if (changed(sc,  localRef.current.clients))  setClients(sc);
      if (changed(st,  localRef.current.tasks))    setTasks(st);
      if (changed(sp,  localRef.current.pages))    setPages(sp);
      if (changed(stg, localRef.current.tags))     setTags(stg);
      if (changed(sm,  localRef.current.meetings)) setMeetings(sm);
      if (changed(sa,  localRef.current.activity)) setActivity(sa);
      try {
        const su = await storage.get("tf-users");
        if (su) {
          const merged = SEED.users.map(seed => { const r = su.find(u => u.id === seed.id); return r ? { ...seed, ...r } : seed; });
          if (JSON.stringify(merged) !== JSON.stringify(localRef.current.users)) setUsers(merged);
        }
      } catch (e) { console.warn("tf-users poll failed", e); }
    }, 15000);
  ...
  // Save only role/allowedClients — never passwords
  useEffect(() => {
    if (!loading) storage.set("tf-users", users.map(u => ({ id: u.id, name: u.name, role: u.role, allowedClients: u.allowedClients || [] })));
  }, [users, loading]);

  const [sidebarOpen, setSidebarOpen] = useState(false);

  // Public token check
  const publicToken  = new URLSearchParams(window.location.search).get("client");
  const publicClient = publicToken ? clients.find(c => c.publicToken === publicToken) : null;

  if (publicClient) {
    const cTasks  = tasks.filter(t => t.clientId === publicClient.id && t.status !== "done");
    const cPages  = pages.filter(p => p.clientId === publicClient.id);
    const typeMap = { post: { label: "Post", color: "#5b6af0" }, reel: { label: "Reel", color: "#e86c4a" }, story: { label: "Story", color: "#f0a030" }, campana: { label: "Campaña", color: "#a855f7" }, otro: { label: "Otro", color: "#64748b" } };
    return (
      <div style={{ minHeight: "100vh", background: "#0d0d0d", fontFamily: "Helvetica Neue, Helvetica, Arial, sans-serif", color: "#f0f0f0", padding: "32px 20px", maxWidth: 860, margin: "0 auto" }}>
        <link href="https://fonts.googleapis.com/css2?family=Lora:wght@400;600;700&display=swap" rel="stylesheet" />
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 32 }}>
          <div style={{ width: 48, height: 48, borderRadius: 12, background: publicClient.color, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 700, fontSize: 18 }}>{publicClient.initials}</div>
          <div><h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, fontFamily: "Lora, Georgia, serif" }}>{publicClient.name}</h1><p style={{ margin: 0, fontSize: 13, color: "#555" }}>Vista compartida — solo lectura</p></div>
        </div>
        <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 12 }}>Tareas activas</h3>
        {cTasks.length === 0 && <p style={{ color: "#555", fontSize: 13 }}>Sin tareas activas.</p>}
        <div style={{ background: "#111111", borderRadius: 14, border: "1.5px solid #222222", overflow: "hidden", marginBottom: 32 }}>
          {cTasks.sort((a,b) => a.dueDate.localeCompare(b.dueDate)).map(t => {
            const s = statusMap[t.status]; const p = priorityMap[t.priority];
            return (
              <div key={t.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 20px", borderBottom: "1px solid #1a1a1a" }}>
                <div style={{ width: 8, height: 8, borderRadius: "50%", background: p.color, flexShrink: 0 }} />
                <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13, fontWeight: 600 }}>{t.title}</div><div style={{ fontSize: 11, color: "#555" }}>Vence: {t.dueDate}</div></div>
                <span style={{ fontSize: 11, fontWeight: 600, color: s.color, background: s.color+"15", padding: "2px 8px", borderRadius: 20 }}>{s.label}</span>
              </div>
            );
          })}
        </div>
        {cPages.length > 0 && <>
          <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 12 }}>Contenido</h3>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(240px,1fr))", gap: 14 }}>
            {cPages.map(p => { const tp = typeMap[p.type] || typeMap.otro; return (
              <div key={p.id} style={{ background: "#111111", borderRadius: 12, border: "1.5px solid #222222", overflow: "hidden" }}>
                <div style={{ height: 4, background: tp.color }} />
                <div style={{ padding: 16 }}><span style={{ fontSize: 10, fontWeight: 700, color: tp.color, background: tp.color+"15", padding: "2px 8px", borderRadius: 20, textTransform: "uppercase" }}>{tp.label}</span><div style={{ fontWeight: 700, fontSize: 14, marginTop: 6 }}>{p.title}</div>{p.content && <p style={{ margin: "8px 0 0", fontSize: 12, color: "#808080", lineHeight: 1.6 }}>{p.content}</p>}</div>
              </div>
            ); })}
          </div>
        </>}
  ...
  if (loading) return <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Lora, Georgia, serif", color: "#707070" }}>Cargando TeamFlow...</div>;
  ...
  const visibleClients = isAdmin
    ...
    : clients.filter(c => {
        ...
        return !u?.allowedClients?.length || u.allowedClients.includes(c.id);
      });
  ...
  const renderContent = () => {
    if (activeSection === "client-detail" && selectedClient)
      return <ClientDetail client={selectedClient} clients={visibleClients} setClients={setClients} tasks={tasks} setTasks={setTasks} pages={pages} setPages={setPages} meetings={meetings} setMeetings={setMeetings} tags={tags} setTags={setTags} users={users} currentUser={currentUser} onBack={() => setActiveSection("clients")} addActivity={addActivity} />;
    switch (activeSection) {
      case "home":     return <Dashboard tasks={tasks} setTasks={setTasks} clients={visibleClients} users={users} tags={tags} setTags={setTags} meetings={meetings} activity={activity} setActiveSection={setActiveSection} currentUser={currentUser} />;
      case "clients":  return <ClientsView clients={visibleClients} setClients={setClients} tasks={tasks} pages={pages} meetings={meetings} setActiveSection={setActiveSection} setSelectedClient={setSelectedClient} />;
      case "tasks":    return <TasksView tasks={tasks} setTasks={setTasks} clients={visibleClients} users={users} tags={tags} setTags={setTags} currentUser={currentUser} addActivity={addActivity} />;
      case "meetings": return <MeetingsView meetings={meetings} setMeetings={setMeetings} clients={visibleClients} users={users} tasks={tasks} setTasks={setTasks} currentUser={currentUser} addActivity={addActivity} />;
      case "workload": return <WorkloadView tasks={tasks} users={users} clients={visibleClients} />;
      case "calendar": return <CalendarView tasks={tasks} clients={visibleClients} />;
      case "activity": return <ActivityView activity={activity} users={users} clients={visibleClients} tasks={tasks} />;
      case "settings": return isAdmin ? <SettingsView users={users} setUsers={setUsers} clients={clients} setClients={setClients} currentUser={currentUser} /> : null;
      default: return null;
    }
  };

  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#0d0d0d", fontFamily: "Helvetica Neue, Helvetica, Arial, sans-serif", color: "#f0f0f0" }}>
      <link href="https://fonts.googleapis.com/css2?family=Lora:wght@400;600;700&display=swap" rel="stylesheet" />
      <style>{`
        * { box-sizing: border-box; }
        body { background: #0d0d0d; color: #f0f0f0; }
        input, textarea, select { background: #1a1a1a !important; color: #f0f0f0 !important; border-color: #2a2a2a !important; }
        ...
        .tf-sidebar { width: 220px; background: #111111; border-right: 1.5px solid #222; display: flex; flex-direction: column; position: sticky; top: 0; height: 100vh; flex-shrink: 0; z-index: 100; transition: transform 0.25s ease; }
        ...
        @media (max-width: 768px) {
          .tf-sidebar { position: fixed; top: 0; left: 0; height: 100vh; transform: translateX(-100%); box-shadow: 4px 0 24px rgba(0,0,0,0.5); }
          .tf-sidebar.open { transform: translateX(0); }
          .tf-hamburger-close { display: flex !important; }
          .tf-topbar { display: flex; align-items: center; justify-content: space-between; padding: 14px 18px; background: #111111; border-bottom: 1.5px solid #222; position: sticky; top: 0; z-index: 90; }
          .tf-overlay { display: block; position: fixed; inset: 0; background: rgba(0,0,0,0.75); z-index: 99; }
      ...
      <div className={`tf-sidebar${sidebarOpen ? " open" : ""}`}>
        <SidebarContent navItems={navItems} activeSection={activeSection} navigate={navigate} setSidebarOpen={setSidebarOpen} currentUser={currentUser} isAdmin={isAdmin} />
      </div>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        ...
        <div className="tf-topbar">
          <button onClick={() => setSidebarOpen(true)} style={{ background: "none", border: "none", cursor: "pointer", color: "#f0f0f0", padding: 4, display: "flex" }}>
            <svg width={22} height={22} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
              <path d="M3 12h18M3 6h18M3 18h18" />
            </svg>
          </button>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ width: 28, height: 28, background: "#1a1a1a", borderRadius: 7, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 13, fontWeight: 700, fontFamily: "Lora, Georgia, serif" }}>T</div>
            <span style={{ fontWeight: 700, fontSize: 15, letterSpacing: "-0.02em", fontFamily: "Lora, Georgia, serif" }}>TeamFlow</span>
          </div>
