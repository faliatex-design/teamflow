...
const SettingsView = ({ users, setUsers, clients, setClients, currentUser }) => {
  ...
  return (
    <div>
      ...
      <p style={{ margin: "0 0 28px", color: "#888", fontSize: 14 }}>Solo visible para administradores.</p>
      <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 14 }}>Usuarios y permisos</h3>
      <div style={{ background: "#fff", borderRadius: 14, border: "1.5px solid #efefef", overflow: "hidden", marginBottom: 32 }}>
        {users.map((u, i) => (
          <div key={u.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 20px", borderBottom: i < users.length-1 ? "1px solid #f5f5f5" : "none" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {u.role !== "admin" && (u.allowedClients||[]).length > 0 && <div style={{ fontSize: 11, color: "#888", marginTop: 3 }}>Clientes: {(u.allowedClients||[]).map(cid => clients.find(c=>c.id===cid)?.name).filter(Boolean).join(", ")}</div>}
            ...
            {u.id !== currentUser.id && <button onClick={() => openEdit(u)} style={{ background: "none", border: "1.5px solid #e8e8e8", borderRadius: 8, padding: "6px 12px", cursor: "pointer", fontSize: 12, fontFamily: "inherit", color: "#555" }}>Editar</button>}
          </div>
        ))}
      </div>
      <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 6 }}>Links públicos por cliente</h3>
      <p style={{ fontSize: 13, color: "#aaa", marginBottom: 14 }}>Compartí las tareas activas y el contenido de un cliente sin necesidad de login.</p>
      <div style={{ background: "#fff", borderRadius: 14, border: "1.5px solid #efefef", overflow: "hidden" }}>
        {clients.map((c, i) => {
          ...
          return (
            <div key={c.id} style={{ padding: "14px 20px", borderBottom: i < clients.length-1 ? "1px solid #f5f5f5" : "none" }}>
              ...
              {link && (
                <div style={{ display: "flex", alignItems: "center", gap: 8, background: "#f7f7f5", borderRadius: 8, padding: "8px 12px" }}>
                  <span style={{ fontSize: 12, color: "#555", flex: 1, wordBreak: "break-all" }}>{link}</span>
                  <button onClick={() => navigator.clipboard.writeText(link)} style={{ background: "none", border: "1.5px solid #e8e8e8", borderRadius: 6, padding: "4px 10px", cursor: "pointer", fontSize: 11, fontFamily: "inherit", color: "#888", whiteSpace: "nowrap" }}>Copiar</button>
                  <a href={link} target="_blank" rel="noreferrer" style={{ background: "none", border: "1.5px solid #e8e8e8", borderRadius: 6, padding: "4px 10px", fontSize: 11, fontFamily: "inherit", color: "#888", textDecoration: "none", whiteSpace: "nowrap" }}>Abrir</a>
                </div>
              )}
            </div>
          );
      ...
      {editingUser && form && (
        <Modal title={`Editar: ${editingUser.name}`} onClose={() => setEditingUser(null)}>
          <div style={{ marginBottom: 16 }}>
            <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#555", marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.04em" }}>Rol</label>
            ...
            <p style={{ margin: "8px 0 0", fontSize: 11, color: "#aaa" }}>{form.role === "admin" ? "Ve y edita todo. Accede a Ajustes." : "Ve solo los clientes que le asignés."}</p>
          </div>
          {form.role === "member" && (
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#555", marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.04em" }}>Clientes visibles</label>
              <p style={{ margin: "0 0 10px", fontSize: 12, color: "#aaa" }}>Sin selección = ve todos los clientes.</p>
    ...
    <div style={{ padding: "16px 14px", borderTop: "1.5px solid #f5f5f5" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
        ...
        <div style={{ minWidth: 0 }}>
          ...
          <div style={{ fontSize: 11, color: "#aaa", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{currentUser.email}</div>
      ...
      <button onClick={() => { localStorage.removeItem("tf-user"); window.location.reload(); }} style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "8px 10px", border: "none", background: "none", cursor: "pointer", color: "#aaa", fontSize: 12, fontFamily: "inherit", borderRadius: 6 }}>
...
export default function App() {
  ...
  if (publicClient) {
    ...
    const typeMap = { post: { label: "Post", color: "#5b6af0" }, reel: { label: "Reel", color: "#e86c4a" }, story: { label: "Story", color: "#f0a030" }, campana: { label: "Campaña", color: "#a855f7" }, otro: { label: "Otro", color: "#64748b" } };
    return (
      <div style={{ minHeight: "100vh", background: "#f7f7f5", fontFamily: "'DM Sans','Helvetica Neue',sans-serif", color: "#1a1a1a", padding: "32px 20px", maxWidth: 860, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 32 }}>
          ...
          <div><h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, fontFamily: "Lora, serif" }}>{publicClient.name}</h1><p style={{ margin: 0, fontSize: 13, color: "#aaa" }}>Vista compartida — solo lectura</p></div>
        </div>
        <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 12 }}>Tareas activas</h3>
        {cTasks.length === 0 && <p style={{ color: "#aaa", fontSize: 13 }}>Sin tareas activas.</p>}
        <div style={{ background: "#fff", borderRadius: 14, border: "1.5px solid #efefef", overflow: "hidden", marginBottom: 32 }}>
          {cTasks.sort((a,b) => a.dueDate.localeCompare(b.dueDate)).map(t => {
            ...
            return (
              <div key={t.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 20px", borderBottom: "1px solid #f8f8f8" }}>
                <div style={{ width: 8, height: 8, borderRadius: "50%", background: p.color, flexShrink: 0 }} />
                <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13, fontWeight: 600 }}>{t.title}</div><div style={{ fontSize: 11, color: "#aaa" }}>Vence: {t.dueDate}</div></div>
        ...
        {cPages.length > 0 && <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(240px,1fr))", gap: 14 }}>
            {cPages.map(p => { const tp = typeMap[p.type] || typeMap.otro; return (
              <div key={p.id} style={{ background: "#fff", borderRadius: 12, border: "1.5px solid #efefef", overflow: "hidden" }}>
                <div style={{ height: 4, background: tp.color }} />
                <div style={{ padding: 16 }}><span style={{ fontSize: 10, fontWeight: 700, color: tp.color, background: tp.color+"15", padding: "2px 8px", borderRadius: 20, textTransform: "uppercase" }}>{tp.label}</span><div style={{ fontWeight: 700, fontSize: 14, marginTop: 6 }}>{p.title}</div>{p.content && <p style={{ margin: "8px 0 0", fontSize: 12, color: "#777", lineHeight: 1.6 }}>{p.content}</p>}</div>
              </div>
            ); })}
          </div>
        </>}
